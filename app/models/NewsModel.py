...
class NewsModel(Model):
    ...
    def create_user(self):
        ...
        if errors:
            ...
            i_query = "INSERT INTO users (first_name, last_name, email, dob, paper_id created_at, password) VALUES (%s, %s, %s, %s, NOW(), %s)"
            i_data = [info['name'], info['email'], info['dob'], p_id, pw_hash]
            self.db.query_db(i_query, i_data)
    ...
    def city_paper(self, info):
        data = [info['name']]
        query = "SELECT * FROM cities JOIN papers ON papers.city_id = cities.id WHERE cities.name = %s"
        paper = self.db.query_db(query,data)
